...
async function createTables(db: { exec: (sqlOrOpts: any) => Promise<any> }) {
  ...
  await db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS albums (id INTEGER PRIMARY KEY AUTOINCREMENT, slug TEXT UNIQUE, title TEXT, description TEXT, cover_image_url TEXT, created_at INTEGER, updated_at INTEGER, published INTEGER DEFAULT 1);
