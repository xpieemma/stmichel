...
import {browser} from '$app/environment';
  ...
  import { stopHeartbeat, adminLocked } from '$lib/auth/heartbeat';
import { clearAuditKey } from '$lib/auth/audit';
 import { syncing, queued } from '$lib/stores/sync-state';
 import { get } from 'svelte/store';
 import SyncIndicator from '$lib/components/admin/SyncIndicator.svelte';
...
<script lang="ts">
  ...
  onMount(() => {
    ...
    // ✅ FIXED: Used get(currentAdmin) instead of $currentAdmin inside onMount
    if (browser && !get(currentAdmin)?.active) {
      goto(resolve('/admin/login'));
    }
