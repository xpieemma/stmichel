<!-- <script lang="ts">
  ...
  // Disable SSR — Leaflet touches `window` at module load time, and
  // Vite would otherwise try to bundle it on the server.
  
  // export const ssr = false;

  onMount(async () => {
    // Defer BOTH the JS module and the CSS so neither hits SSR.
   ...
   L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    ...
    if (db) {
      ...
      pois.forEach((poi) => {
        if (poi.lat && poi.lng) {
          // Build the popup as a real DOM node and set text via
          // .textContent so admin-supplied title/description CAN'T
          // inject HTML or event handlers. bindPopup accepts a node
          // directly — no innerHTML round-trip.
