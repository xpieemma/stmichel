...
ALTER TABLE admin_allowlist ADD COLUMN email TEXT;
ALTER TABLE admin_allowlist ADD COLUMN status TEXT
  CHECK(status IN ('pending', 'approved', 'rejected', 'bootstrap'))
  DEFAULT 'approved';
ALTER TABLE admin_allowlist ADD COLUMN requested_at INTEGER;
ALTER TABLE admin_allowlist ADD COLUMN reviewed_at INTEGER;
ALTER TABLE admin_allowlist ADD COLUMN reviewed_by TEXT;
...
UPDATE admin_allowlist SET status = 'bootstrap'
  WHERE username = 'admin' AND added_by = 'system';
